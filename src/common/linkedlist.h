...
/*!
  ...
  \return 0 if the node was found and deleted, 1 otherwise
*/
int node_delete(Node **head, Node *node, int(*remove_data)(void *data));

/*!
  \fn Node *find(Node *head, int(*compare)(const void *, const void *), const
  				void *key)
...
/**
   Create a duplicate of list l and assign it to new.
   The data is not duplicated.

   @param l the list to be duplicated
   @param new the new list, which will point to the allocated list.

   @return a valid pointer to a dynamically allocated linked list with the
   duplicated data.  NULL otherwise.
*/
Node *node_dup_list(Node *l, Node **new);
